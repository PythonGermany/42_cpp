...
AForm::AForm(std::string name, int sR, int const eR)
    : name(name), signReq(sR), exeReq(eR) {
...
AForm::AForm(AForm const& rhs)
    : name(rhs.name), signReq(rhs.signReq), exeReq(rhs.exeReq) {
...
bool const& AForm::getSigned() const { return (isSigned); }

int const& AForm::getSignReq() const { return (signReq); }

int const& AForm::getExeReq() const { return (exeReq); }
